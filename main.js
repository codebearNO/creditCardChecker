...
const validateCred = (array) => {
	...
	for (let i = array.length - 1; i >= 0; i -= 2) {
		if (i > 0) tempArray.push(array[i - 1] * 2);
		tempArray.push(array[i]);
	}

	let resultArray = tempArray.map((number) =>
		number > 9 ? number - 9 : number
	);

	resultArray.forEach((number) => (sumOfArray += number));
...
const idInvalidCardCompanies = (invalidCards) => {
	...
	for (let card of invalidCards) {
		switch (card[0]) {
			case 3:
				!invalidCardCompanies.includes("Amex (American Express)")
					? invalidCardCompanies.push("Amex (American Express)")
					: "";
				break;
			case 4:
				!invalidCardCompanies.includes("Visa")
			...
			case 5:
				...
				break;
			case 6:
				!invalidCardCompanies.includes("Discover")
